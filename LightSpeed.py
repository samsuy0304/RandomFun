...
dist_m = np.array([40*0.3,36*0.3,30*0.3,24*0.3,18*0.3])
fig0 = plt.figure(figsize=(8,4))
fig0.patch.set_facecolor('xkcd:black')
plt.style.use('dark_background')
ax0=plt.subplot(1,2,1)
ax0.plot(time_differences_averages,dist_m*2,'ro')
#plt.xlabel('$\Delta$ t')
#plt.ylabel('Distance in meters')
print('this should be linear wtf???? is going on????')
plt.show()
...
data_x = np.array(time_differences_averages)
data_y = dist_m
#ysig
...
m = Minuit(least_squares, 300000000)
...
#pprint(m.covariance())
print(' ')
#plt.plot(model(data_x, *m.values), 'r-')
